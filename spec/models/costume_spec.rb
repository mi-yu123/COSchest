...
RSpec.describe Costume, type: :model do
  ...
  let(:costume) { build(:costume, user: user) }
    ...
    describe 'image のバリデーション' do
      ...
      context 'imageの形式' do
        ...
        it 'imageが png 形式の場合' do
          costume = build(:costume, :with_png_image)
          expect(costume).to be_valid
        end

        it 'imageが jpeg png ではない場合' do
          costume = build(:costume, :with_invalid_image)
          costume.valid?
          expect(costume.errors[:image]).to include('ファイルの形式はJPEGまたはPNGである必要があります。')
        end
      end
  ...
  describe 'ransackable_attributes' do
    it 'ransack によって使用可能な属性を返す' do
      expect(Costume.ransackable_attributes).to include('character_name', 'status')
    end
  end

  describe 'ransackable_associations' do
    it 'ransack によって使用可能な関連モデルを返す' do
      expect(Costume.ransackable_associations).to be_empty
    end
  end

  describe '#resize_image' do
    let(:costume) { create(:costume, :with_jpeg_image) }

    context 'imageがある場合' do
      it 'imageをリサイズする' do
        expect(costume.resize_image).to be_present
      end
    end
